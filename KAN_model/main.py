...
def create_kan_model(input_shape, num_classes):
    # Input layer that receives data with the given shape (e.g., spectrogram input)
    inp = tf.keras.layers.Input(shape=input_shape)

    # Start the processing pipeline from the input
    x = inp

    # Block 1 - Initial convolutional block to extract low-level features
    for _ in range(2):
        # 2D Convolution with 64 filters, 3x3 kernel, L2 regularization
        x = tf.keras.layers.Conv2D(64, kernel_size=(3, 3), padding='same',
                                   kernel_regularizer=tf.keras.regularizers.l2(0.001))(x)
        # Normalize feature maps to speed up convergence
    ...
    # Block 2 - Intermediate block for more abstract feature extraction
    for _ in range(2):
    ...
    # Block 3 - Deeper layer to capture higher-level patterns
    for _ in range(2):
        x = tf.keras.layers.Conv2D(256, kernel_size=(3, 3), padding='same',
                                   kernel_regularizer=tf.keras.regularizers.l2(0.001))(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Activation("relu")(x)
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)

    # Block 4 - Deepest convolutional block for fine-grained feature learning
    for _ in range(2):
        x = tf.keras.layers.Conv2D(512, kernel_size=(3, 3), padding='same',
                                   kernel_regularizer=tf.keras.regularizers.l2(0.001))(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Activation("relu")(x)
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)

    # Global pooling to condense the feature maps into a compact representation
    x = tf.keras.layers.concatenate([
        tf.keras.layers.GlobalAveragePooling2D()(x),  # Captures average activations
        tf.keras.layers.GlobalMaxPooling2D()(x)       # Captures strongest activations
    ])

    # Fully connected dense layer to integrate all extracted features
...
def main_func(train_dir, test_dir, batch_size, epochs, output_dir, plot_confusion_matrix_flag, plot_accuracy_flag, plot_f1_scores_flag):
    ...
    # y_train = np.expand_dims(y_train, axis=-1)  # If needed, adjust to match the output dimensions
