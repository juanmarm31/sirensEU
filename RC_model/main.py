...
def create_reservoir_model(input_shape, num_classes, reservoir_size=100):
    inp = tf.keras.layers.Input(shape=input_shape)

    # Reshaping input for the RC model (flatten the input data)
    x = tf.keras.layers.Reshape((-1, np.prod(input_shape)))(inp)
    
    # Reservoir Layer - Simulating a fixed random recurrent network (reservoir)
    # For RC, the reservoir is often just a large random recurrent matrix, which is fixed.
    # Here we use a simple LSTM as an approximation to simulate the reservoir.

    # Using an LSTM layer to simulate a random fixed recurrent reservoir
    reservoir = tf.keras.layers.LSTM(reservoir_size, activation=None, use_bias=False, return_sequences=False)(x)
    
    # Readout Layer: A simple dense layer to map the reservoir's output to class probabilities
    readout = tf.keras.layers.Dense(num_classes, activation='softmax')(reservoir)

    model = tf.keras.models.Model(inputs=inp, outputs=readout)
...
def main_func(train_dir, test_dir, batch_size, epochs, output_dir, plot_confusion_matrix_flag, plot_accuracy_flag, plot_f1_scores_flag):
    ...
    # Build model
    model = create_reservoir_model(input_shape, num_classes)
    model.summary()
